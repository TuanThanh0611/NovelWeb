...
import { type PartialOrValue } from 'ngxtension/connect';
...
type ActionStreams<TSignalValue, TActionSources extends NamedActionSources<TSignalValue>> = {
    [K in keyof TActionSources & string as `${K}$`]: TActionSources[K] extends ActionSourceFn<TSignalValue, unknown> ? Observable<void> : TActionSources[K] extends ActionSourceFn<TSignalValue, infer TValue> ? TValue extends Observable<any> ? TValue : Observable<TValue> : never;
};
type InitialStateStreams<TSignalValue> = {
    [K in keyof TSignalValue & string as `${K}$`]: Observable<TSignalValue[K]>;
};
type ActionUpdates<TSignalValue, TActionSources extends NamedActionSources<TSignalValue>> = {
