...
function getOutputInitializer(propertyName, currentType, decorator, initializer) {
    ...
    const initializerOrType = initializer !== null && initializer !== void 0 ? initializer : (typeof currentType === 'string' ? currentType : undefined);
    if (!initializerOrType) {
        devkit_1.logger.error(`[ngxtension] Unable to find initializer or type for "${propertyName}"`);
    ...
    if (!initializerOrType.includes('EventEmitter')) {
        if (!initializer) {
            devkit_1.logger.error(`[ngxtension] Unable to find initializer for "${propertyName}"`);
            return (0, node_process_1.exit)(1);
        }
        ...
        if (initializerOrType.includes('EventEmitter()')) {
            ...
            const genericTypeOnEmitter = initializerOrType.match(/EventEmitter<(.+)>/);
                        ...
                        if (outputDecorator) {
                            ...
                            if (!initializer) {
                                ...
                                if (constructor) {
                                    ...
                                    constructorInitializer === null || constructorInitializer === void 0 ? void 0 : constructorInitializer.remove();
                                }
                            }
